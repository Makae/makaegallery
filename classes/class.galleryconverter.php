...
class GalleryConverter
{
    ...
    {
        $to_width = false;
        if ($config->getResizeMode() == GalleryConverter::RESIZE_MODE_NO_RESIZE) {
            return 1;
        } else if ($config->getResizeMode() == GalleryConverter::RESIZE_MODE_TO_SMALLER) {
            $to_width = $width < $height;
        } else if ($config->getResizeMode() == GalleryConverter::RESIZE_MODE_TO_DEFINED_DIMENSION) {
